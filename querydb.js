...
async function  createTodo(data, res){
    ...
    Todos.create({username: data.username, password: hashedPassword, email: data.email}).then((todo) => {
        res.status(201).send(`${todo.username}'s account has been succesfully created.`);
    }).catch((err) => {
        res.status(400).send(err.message + ": looks like you have an account with us");
    });
}

//query database
function queryTodos(){
    Todos.findAll().then((res) => {
        console.log(res);
    }).catch((err) => {
...
function queryTodo(data){
    Todos.findOne({where: data}).then((res) => {
        console.log(res);
    }).catch((err) => {
